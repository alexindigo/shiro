...
Game.prototype.addAnswer = function Game_addAnswer(data, callback)
{
  ...
  // add answer
  teamData.answers[question] = {text: data.text, time: diff, spent: timeSpent};

  // save
  this.save('team', teamData.login, teamData, function(err)
  {
    if (err) return callback({code: 500, message: err});
    return callback(null, {team: teamData.login, question: question, text: data.text, time: diff, spent: timeSpent});
  });
}
...
// send different data to the admin and to others
      ...
      _sockets.write({ 'team:answer': _.omit(answer, 'text') });
    });
  };
