...
Game.prototype.init = function Game_init(callback)
{
  ...
  level(this._storage, {keyEncoding: 'utf8', valueEncoding: 'json'}, function(err, db)
  {
    ...
    }, function(err, res)
    {
      ...
      _game.meta     = res.meta;
      // reset online flag
      _game.team     = _.each(res.team, function(team){ team.online = false; });
...
Game.prototype.addTeam = function Game_addTeam(team, callback)
{
  ...
  if (!admin[team.socketid])
  {
    return callback({code: 403, message: 'You can add teams to the game.'});
  }
  ...
  this.events['helo'] = function Game__initEventListeners_helo(socket, data)
  {
    ...
    socket.write(
    {
      ...
      { instance: _game.meta.instance
        // send upstairs only logins, names and online flag
      , teams   : _.map(_game.team, function(team){ return {login: team.login, name: team.name, online: team.online, points: team.points}; })
      , state   : _game.state
      }
    });
