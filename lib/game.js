...
Game.prototype._sendState = function Game__sendState(socket)
{
  ...
  socket.write(
  {
    ...
    , teams    : _.map(this.team, function(team){ return _.transform(team, function(result, item, key){ if (key == 'password') return; result[key] = typeof item != 'object' ? item : _.transform(item, function(result, item, key){ result[key] = admin[socket.id] ? item : _.omit(item, 'text'); }); }); })
