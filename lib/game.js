...
Game.prototype.addAnswer = function Game_addAnswer(data, callback)
{
  ...
  timeSpent = 60000 - (diff[0] * 1000 + Math.floor(diff[1] / 10e6));

  // add answer
  teamData.answers[question] = {text: data.answer.text, time: diff, spent: timeSpent, correct: false};

  // save
  this.save('team', teamData.login, teamData, function(err)
  {
    if (err) return callback({code: 500, message: err});
    return callback(null, {team: teamData.login, question: question, text: data.answer.text, time: diff, spent: timeSpent});
  });
}
...
Game.prototype._initEventListeners = function Game__initEventListeners()
{
  ...
  this.events['helo'] = function Game__initEventListeners_helo(socket, data)
  {
    ...
    socket.write(
    {
      ...
      , teams    : _.map(_game.team, function(team){ return _.transform(team, function(result, item, key){ if (key == 'password') return; result[key] = typeof item != 'object' ? item : _.transform(item, function(result, item, key){ if (key == 'text') return; result[key] = item; }); }); })
      , questions: _.map(_game.question, function(question){ return _.omit(question, ['text', 'answer']); })
      , state    : _.transform(_game.state, function(result, item, key){ result[key] = (item && key == 'timer' ? _.omit(item, 'countdown') : item); })
      }
    });
  ...
  this.events['team:answer'] = function Game__initEventListeners_team_answer(socket, data)
  {
    ...
    _game.addAnswer({login: login, answer: data}, function Game_addAnswer_callback(err, answer)
    {
      ...
      _.each(_sockets.connections, function(s, id)
      {
        if (admin[id])
        {
          s.write({ 'team:answer': answer });
        }
        else
        {
          s.write({ 'team:answer': _.omit(answer, 'text') });
        }
      });
    });
  };
