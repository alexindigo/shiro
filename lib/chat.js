...
Chat.prototype.left = function Chat_left(user, callback)
{
  ...
  {
    return callback({code: 400, message: 'Missing data.', sid: user.socketid, so: sockets[user.socketid], sos: sockets});
  }
...
{
  var timeLeft = 5
    , user = sockets[message.socketid].nickname
    ;
  ...
  setInterval(function()
  {
    ...
    _chat._sockets.write({ 'chat:error': {err: {code: 418, message: 'User '+user+' asked for a full server shutdown. Server will be shutdown in '+timeLeft+' seconds.'}, origin: 'message'} });
  ...
  return callback({code: 418, message: 'User '+user+' asked for a full server shutdown. Server will be shutdown in '+timeLeft+' seconds.'});
}
// !!!
